...
PetscInt OutMaskCountActive(OutMask *omask)
{
	...
	if(omask->velocity_old)   cnt++; // previous-step velocity
...
//	ierr = getIntParam   (fb, _OPTIONAL_, "out_ang_vel_mag",    &omask->ang_vel_mag,       1, 1); CHKERRQ(ierr);
	...
	if(omask->velocity_old)   PetscPrintf(PETSC_COMM_WORLD, "   Previous-step velocity (velocity_old)   @ \n");
